...
RUN dnf upgrade -y && dnf install -y \
    cmake gcc pkg-config glib2-devel json-glib-devel libcurl-devel openssl-devel wget && \
    dnf clean all
...
RUN dnf upgrade -y && dnf install -y \
    glib2 json-glib libcurl openssl && \
    dnf clean all
...
COPY --from=build /app/build/abls-agent-sms /usr/local/bin/abls-agent-sms

ENV ABLS_IN_A_CONTAINER=1
CMD ["/usr/local/bin/abls-agent-sms"]
